...
contract Auction {
    ...
    string public publicKey =
        "MEkwDQYJKoZIhvcNAQEBBQADOAAwNQIuAM663sfXuONaQNWNgP4lhZRLfbF13+BgnZ55pUhcWDDdHvxEwpH24ytm3o/TSwIDAQAB";
    ...
    constructor(
        ...
        uint _winner_pay,
        uint _destroy,
        bytes32 _product_description,
        bytes32 _technical_specification,
    ...
    function bidder_participation(bytes32 _RFI_form, bytes32 _RFQ_form) public {
        ...
        require(
            Bids[msg.sender].existing == true,
            "the bidder must existing! "
        );
    ...
    function getBidder_details(
        address _address
    ) external view onlyauctioneer returns (bytes32, bytes32) {
    ...
    function Bid(string cipher, bool pVerify) public payable returns (uint) {
        ...
        require(
            (block.number < submit && block.number > init) || testing,
            "不在投标时间段内"
        );
    ...
    function ClaimWinner(
        ...
        require(
            (block.number < verify && block.number > submit) || testing,
            "不在决出获胜者时间段内"
        );
    ...
    function Reveal() external view returns (uint, string, bytes32) {
        ...
        return (
            highest_bid,
            Bids[winning_bidder].cipher,
            Bids[winning_bidder].proof
        );
    }
    ...
    function Verify(bool proofVerify, string _cipher) external {
        ...
        require(
            keccak256(abi.encodePacked(_cipher)) ==
                keccak256(abi.encodePacked(Bids[msg.sender].cipher)),
            "承诺验证未通过"
        );
        winner_cipher = true;
    }
    ...
    function WinnerPay() public payable {
        require(
            (bidder_win &&
                (block.number < winner_pay && block.number > verify)) ||
                testing,
            "未到投标者胜利时期"
        );
        ...
        require(
            msg.value >= highest_bid - fairnessFees,
            "账户余额不足已支付获胜价格"
        );
        bidder_win = false;
        withdrawLock = true;
    ...
    function Withdraw() public {
        ...
        require(
            Bids[msg.sender].paidBack == false &&
                Bids[msg.sender].existing == true,
            "账户金额已退还或账户不存在"
        );
        require(
            withdrawLock || block.number > winner_pay || testing,
            "还未到退还时间"
        );
